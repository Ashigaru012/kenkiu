...
int main(int argc,char **argv){
    ...
    char packet_kind[2][10] = {"normal","attack"};
    ...
    xbee_end_device(dev,SLEEP_TIME,8,0);                       //ZEDのスリープ設定
    ...
    if(xbee_ping(dev) == 0xFF){
        while(1){
            ...
            printf("正常パケットの送信回数は%dです\n正常パケットの破棄された回数は%dです。\n正常パケットの破棄率は%lfパーセントです\n",normal_cnt,normal_packet_cnt,((float)normal_packet_cnt / (float)normal_cnt) * 100 );
